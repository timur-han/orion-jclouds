...
public class SameFileWithDiffTypeFallback implements Fallback<Boolean> {
	...
	@Inject
	public SameFileWithDiffTypeFallback(OrionApi api,
	...
	public Boolean createOrPropagate(Throwable t) throws Exception {
		...
		if (t instanceof OrionResponseException) {
			...
			if (exception.getError().getHttpCode().equals("500")) {
				...
				String path = exception.getCommand().getCurrentRequest()
						.getEndpoint().getRawPath();
				while (path.startsWith(OrionConstantValues.PATH_DELIMITER)) {
					path = path.replaceFirst(
							OrionConstantValues.PATH_DELIMITER, "");
				}
				path = path
						+ exception.getCommand().getCurrentRequest()
								.getHeaders().get(OrionHttpFields.HEADER_SLUG)
								.toArray()[0];
				boolean result = api.deleteGivenPath(path);

				if (result) {
					exception.getCommand().setException(null);
					if (String.valueOf(
							commandExecutor.invoke(exception.getCommand())
