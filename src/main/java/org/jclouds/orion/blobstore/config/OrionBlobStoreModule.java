...
public class OrionBlobStoreModule extends AbstractModule {
	...
	protected void configure() {
		...
		bind(DeleteDirectoryStrategy.class).to(org.jclouds.orion.blobstore.strategy.MarkersDeleteDirectoryStrategy.class).in(Scopes.SINGLETON);
		bind(PutBlobsStrategy.class).to(PutBlobsStrategyImpl.class).in(Scopes.SINGLETON);
		bind(ClearContainerStrategy.class).to(DeleteAllKeysInList.class).in(Scopes.SINGLETON);
