...
public class ConvertToFlatList implements Function<JsonNode, List<OrionChildMetadata>> {
	...
	public List<OrionChildMetadata> apply(JsonNode parentNode) {
		final List<OrionChildMetadata> arrayList = new ArrayList<OrionChildMetadata>();
		
		if (parentNode.has(OrionConstantValues.LIST_CHILDREN)
				&& parentNode.get(OrionConstantValues.LIST_CHILDREN).isArray()) {
			for (final JsonNode childNode : parentNode.get(OrionConstantValues.LIST_CHILDREN)) {
				try {
					final OrionChildMetadata childData = this.mapper.readValue(childNode, OrionChildMetadata.class);
				...
				arrayList.addAll(new ConvertToFlatList(this.mapper).apply(childNode));
			}
		}
